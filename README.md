...
左親指の追加音 `n/t/k/nt/nk/tk/ntk` をレジストリ名として、出力文字列を記録・再生できます。

| 入力 | 役割 |
| --- | --- |
| `#n-*` (例) | レジストリ `n` の記録開始。すでに記録中なら停止。|
| `#n-*` (例) | レジストリ `n` の記録停止。未記録時は記録開始。|
| `#-*` | 直近に開始した記録中レジストリを1つ停止。|
| `#n` (例) | レジストリ `n` の再生。|

記録内容は「実際に出力された文字列」です。
同じレジストリで再度 `#n-*` を開始すると、そのレジストリ内容は上書きされます。
