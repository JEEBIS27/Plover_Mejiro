...
また、その際は他の略語と競合がなければ`*`を省略することもできます
`KAn-KNA` `-SKNI` → 「鑑みる」
`KAn-KNA` `-IA` → 「考える」
...
このプラグインをインストールしたあと、一度Ploverを再起動した後、Ploverメニューの歯車マーク``Configuration``から``Systems``タブを開き、``Mejiro``システムを選択して起動します
